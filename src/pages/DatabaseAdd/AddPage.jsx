...
export default function AddPage() {
    ...
    useEffect(() => {
        if (searchValue.length > 1) {
            fetch("http://localhost:8080/rowsFilter/" + searchValue)
    ...
    function onSubmit() {
        ...
        const requestOptions = {
          ...
          fetch('http://localhost:8080/pages/', requestOptions)
            .then((response) => {
                setInput({appName: ""})
                setInput({rowsId: []})
                setElements([])
    ...
    return(
        <div className="p-3">
            <p className="text-white font-bold text-5xl w-full text-start py-5">Pages</p>
            <div className='pb-4'>
                <label for="title" class="block mb-2 text-start dark:text-white font-apple text-white text-xl font-bold">App Name</label>
